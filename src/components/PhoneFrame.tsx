...
/** Height of the drawn status bar, in the viewport's own logical pixels. */
export const statusBarHeight = 48

type PhoneFrameProps = {
  viewport: DemoViewport
  /** Background behind the status bar — set it to the screen's own canvas color. */
  canvas?: string
  children: ReactNode
}

/**
 * The device chrome shared by the live demo and the static walkthrough shots,
 * so a screenshot on the page reads as the same device as the prototype above
 * it. Content is measured against the viewport and scaled to whatever width the
 * frame is given; --demo-scale carries that ratio to the CSS.
 */
export function PhoneFrame({ viewport, canvas = '#ffffff', children }: PhoneFrameProps) {
